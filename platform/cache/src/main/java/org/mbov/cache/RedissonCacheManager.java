/*
 * 文 件 名:  RedissionCacheManager.java
...
public class RedissonCacheManager implements CacheManager, ResourceLoaderAware, InitializingBean {
	...
	public RedissonCacheManager(RedissonClient redisson, Map<String, ? extends CacheConfig> config,
			CacheRelationHandler cacheRelationHandler) {
		this(redisson, config, null, cacheRelationHandler);
	}
	...
	public RedissonCacheManager(RedissonClient redisson, String configLocation,
			CacheRelationHandler cacheRelationHandler) {
		this(redisson, configLocation, null, cacheRelationHandler);
	}
	...
	private Cache createMap(String name, CacheConfig config) {
		...
		Cache cache = new RedissonCache(map, allowNullValues, name, cacheRelationHandler, redisson);
	...
	private Cache createMapCache(String name, CacheConfig config) {
		...
		Cache cache = new RedissonCache(map, config, allowNullValues, name, cacheRelationHandler, redisson);
		Cache oldCache = instanceMap.putIfAbsent(name, cache);
		if (oldCache != null) {
			cache = oldCache;
		} else {
